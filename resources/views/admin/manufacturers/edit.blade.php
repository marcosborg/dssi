                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.manufacturers.update", [$manufacturer->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('url') ? 'has-error' : '' }}">
                            <label class="required" for="url">{{ trans('cruds.manufacturer.fields.url') }}</label>
                            <input class="form-control" type="text" name="url" id="url" value="{{ old('url', $manufacturer->url) }}" required>
                            @if($errors->has('url'))
                        ...
                        <div class="form-group {{ $errors->has('text_pt') ? 'has-error' : '' }}">
                            <label for="text_pt">{{ trans('cruds.manufacturer.fields.text_pt') }}</label>
                            <textarea class="form-control ckeditor" name="text_pt" id="text_pt">{!! old('text_pt', $manufacturer->text_pt) !!}</textarea>
                            @if($errors->has('text_pt'))
                                <span class="help-block" role="alert">{{ $errors->first('text_pt') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.manufacturer.fields.text_pt_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('text_en') ? 'has-error' : '' }}">
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.manufacturer.fields.text_en_helper') }}</span>
                        </div>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.manufacturers.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $manufacturer->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
