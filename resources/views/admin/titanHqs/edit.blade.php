...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.titan-hqs.update", [$titanHq->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="product_number">{{ trans('cruds.titanHq.fields.product_number') }}</label>
                <input class="form-control {{ $errors->has('product_number') ? 'is-invalid' : '' }}" type="text" name="product_number" id="product_number" value="{{ old('product_number', $titanHq->product_number) }}">
                @if($errors->has('product_number'))
                    <div class="invalid-feedback">
                        {{ $errors->first('product_number') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.titanHq.fields.product_number_helper') }}</span>
            </div>
            <div class="form-group">
